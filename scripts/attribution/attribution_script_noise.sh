...
for pt in 2.5 3 3.5 4 4.5 5 5.5 6; do
  ...
  pt_str=$(echo $pt | sed 's/\.//')
	...
	for nr in 0.01 0.02 0.05 0.1 0.25 0.5; do

		echo PLEC cutoff $pt, noise level $nr...

		nr_str=$(echo $nr | sed 's/\.//')


		features_npy=features_plec_${pt_str}
        	labels_npy=labels_plec_${pt_str}
        	results_name_plec=plec_performance_${pt_str}_${nr_str}
        	results_name_morgan=morgan_performance_${pt_str}_${nr_str}
        	model_fname=/tmp/rf_model_plec_${pt_str}.joblib
        	attr_dir=attributions_dir_${pt_str}_${nr_str}


        	python fit_rf.py ${root_dir} -f ${features_npy} -l ${labels_npy} --write_results -pof ${results_name_plec} -mof ${results_name_morgan} --model_fname ${model_fname} -nr ${nr}
        	parallel "python compute_attributions_saved_model.py ${root_dir_test} ${model_fname} ${lig_root}{}.sdf ${pharm_root}{}.sdf -pt ${pt} -s {} -ad ${attr_dir}  " ::: {0..500}
